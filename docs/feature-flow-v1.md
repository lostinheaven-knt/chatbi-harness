# ChatBI Harness Cycle 1 Feature Flow (v1)
...
10. **`docs/harness/README.md` not present.** `docs/dev-cycle-1.md` Task 5 lists
    `docs/harness/README.md` as an expected file, but it is absent from the
    Cycle 1 inventory (only `compatibility.md`, `configuration.md`,
    `installation.md` exist under `docs/harness/`). This is a Task 5 ownership
    gap, not a Task 6 deliverable; flagged here for the orchestrator.
...
```
Actual result: 42 files listed. The Cycle 1 deliverables are all present:
`CLAUDE.md`, `CONTEXT.md`, 3 rules, `chatbi-init.md`, `settings.json`,
...
config fixtures, 3 harness docs, 7 test files. Note: `docs/harness/README.md`
is absent (gap 10 above). No `__pycache__` or `*.pyc` residue was found
(`find -name __pycache__ -o -name '*.pyc'` returned nothing).
