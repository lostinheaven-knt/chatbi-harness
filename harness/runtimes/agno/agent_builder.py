...
_GOVERNANCE_PROTOCOL = (
    ...
    "or 'latest_available_day' when the question has no temporal scope), "
    ...
    "(REQ-001 clarify). Never invent a calendar window "
    "(e.g. 2024-06-01_to_2024-06-30) without a user or data basis; "
    ...
    "5.1 SESSION CALIBER: when the operator confirms a working scope "
    "(e.g. 15 creator-grain branches vs 17-table inventory), call "
    "chatbi_record_evidence with kind=operator-adjudication and the "
    "pinned list (accepted, rejected, who confirmed). Do NOT rewrite it "
    "as a published metric or curated-reference T2. Do NOT write "
    "semantic/. Same-session later runs inherit the pin — do not ask "
    "again unless the operator explicitly changes it.\n"
    "5.2 UNPUBLISHED DRAFTS: chatbi_dbt_draft writes models/ only. "
    "Until dbt execute + owner publish, those files are drafts, not "
    "governed T2. Do not write semantic/ from analyze, T3 exploration, "
    "or dbt_draft. Semantic docs go through maintain-knowledge after "
    "owner confirmation. Other sessions' unpublished probes are not "
    "your authority.\n"
    ...
    "operator approves, record "
    "chatbi_record_evidence(kind=operator-adjudication, content="
    "{{approve_execute: true, models: <select list>}}) then call "
    "chatbi_dbt_execute — do NOT wait for an analysis-review PASS on "
    "the SQL files (the hook accepts the session pin + draft SHAs). "
    "Then analyze from the new governed models. T3 raw is allowed ONLY "
    ...
    "7.6. BUILD-PHASE SHORTEST PATH: after source_inventory.json exists "
    "and models/ still has no .sql, call chatbi_build_plan next — do NOT "
    "list_files, search_files, search_content, chatbi_crosscheck, or "
    "chatbi_query_source. On the bootstrap turn use "
    "bootstrap.tables[].name / build_plan.source_tables[].name — do NOT "
    "read_file the inventory and do NOT chatbi_query_source for table "
    "names. STOP after chatbi_build_plan: present the plan and wait for "
    "the operator to confirm caliber. Do NOT "
    "chatbi_dbt_draft, chatbi_dbt_execute, chatbi_query_source, "
    "chatbi_submit_candidate, or chatbi_review on that turn. After the "
    "operator confirms caliber in a later message, in THAT assistant "
    "turn first read_file .chatbi/bootstrap/source_inventory.json once "
    "(column names for planned tables; types omitted; do not "
    "query_source to discover columns), then chatbi_dbt_draft once per "
    "planned model (parallel "
    "ok) with relative_path=models/{{layer}}/{{name}}.sql (copy "
    "build_plan.models[].relative_path; never ods/... without models/) "
    "and STOP for execute approval. Resume chatbi_query_source / "
    "chatbi_crosscheck only after governed SQL models exist, and only "
    "when freezing a candidate (7.5).\n"
    ...
    "freshness | owner | confidence | locatable refs: models/**.sql "
    "and/or evidence:<recorded content_sha256>>\"}}. Cite ONLY the "
    "content_sha256 field returned by chatbi_query_source / "
    "chatbi_record_evidence (never statement_sha256, never a guessed "
    "hash). Every numeric cell in answer_body must appear in a recorded "
    "evidence payload, or label it Agent interpretation (RAW-003). A "
    "footer with only tier/round/freshness/owner/confidence is ANS-002 "
    "and will be denied at submit — do not chatbi_review until submit "
    "succeeds. Then call "
    "chatbi_review. After "
    ...
    *,
    ...
    pre_hooks = normalize_pre_hooks([
        ...
        ChatbiPolicyGuardrail(config=config, event_log=event_log,
                              deployment=deployment, run_scope=scope),
    ])
