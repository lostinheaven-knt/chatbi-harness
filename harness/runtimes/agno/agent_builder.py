...
_GOVERNANCE_PROTOCOL = (
    ...
    "5. FRESHNESS CHECKPOINT (QLT-001): before submitting any candidate, "
    "verify the requested time window against the governed data's "
    "freshness. When the window CANNOT be verified — e.g. the source is a "
    "single-point snapshot while the request is a date range — STOP and "
    "ASK the user to choose: (a) proceed with the point-in-time caliber "
    "(T3 raw exploration, low confidence, high-risk disclosure ANS-003), "
    "(b) adjust the request window to what the data can support, or "
    "(c) propose a source extension (chatbi-bootstrap, only after operator "
    "confirmation). A single-day snapshot is NOT an error — it only means "
    "the answer must state the point-in-time caliber and must not claim "
    "range coverage. Do NOT submit a candidate until the user confirms a "
    "caliber — never deliver an unverifiable-window answer unprompted.\n"
